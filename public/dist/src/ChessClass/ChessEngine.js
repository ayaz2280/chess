...
class ChessEngine {
    static legalMovesCache = new node_cache_1.default({ stdTTL: 60, checkperiod: 120, useClones: false });
    static initGame(player, opponent) {
        const gameState = {
            player: player === 'human' ? new Player_1.HumanPlayer('white') : new Player_1.ComputerPlayer('white'),
            opponent: opponent === 'human' ? new Player_1.HumanPlayer('black') : new Player_1.ComputerPlayer('black'),
    ...
    static requestCastlingRights(gameState) {
        ...
        (0, console_1.assert)(whiteKingPositions.length === 1 && blackKingPositions.length === 1);
    ...
    static getPawnMoves(gameState, pos, types) {
        ...
        if (!types ||
            ...
            if (this.isFirstMove(gameState, pos)) {
                ...
                if (twoSquareAhead &&
                    ...
                    entry = this.buildHistoryEntry(gameState, move, null, 'move');
                    if (entry) {
                        moves.push(entry);
                    }
                }
        ...
        if (!types ||
            ...
            if (leftDiagonal) {
                if (this.canAttackSquare(gameState, pos, leftDiagonal, leftDiagonalOffset)) {
                    ...
                    entry = this.buildHistoryEntry(gameState, move, this.getDestroyedPiece(gameState, piece, move), 'attackMove');
                    if (entry) {
                        moves.push(entry);
                    }
                }
        ...
        if (!types || types.includes('enPassant')) {
            ...
            if (enPassantPos) {
                ...
                entry = this.buildHistoryEntry(gameState, move, this.getFigure(gameState, this.getPositionRelativeTo(enPassantPos, dir, { x: 0, y: -1 })), 'enPassant');
                if (entry) {
                    moves.push(entry);
                }
            }
    ...
    static promotePawn(gameState, pos, pieceOfChoice) {
        ...
        const HistoryEntry = this.buildHistoryEntry(gameState, emptyMove, null, 'promotion');
        if (HistoryEntry) {
    ...
    static getKnightMoves(gameState, pos, types) {
        ...
        for (const endPos of pseudoLegalKnightPositions) {
            ...
            if (!destroyedPiece && (!types || types.includes('move'))) {
                moves.push(this.buildHistoryEntry(gameState, this.getMove(pos, endPos), null, 'move'));
            }
            if (destroyedPiece && (!types || types.includes('attackMove'))) {
                moves.push(this.buildHistoryEntry(gameState, this.getMove(pos, endPos), destroyedPiece, 'attackMove'));
            }
        }
        return moves;
    }
    ...
    static getCastlingMoves(gameState, kingPos) {
        ...
        if (leftRookOnPlace) {
            ...
            if (castlingPossible) {
                ...
                moves.push(this.buildHistoryEntry(gameState, move, null, 'castling'));
            }
        }
    ...
    static getEnPassantPos(gameState, pos) {
        ...
        console.log(lastHistoryEntry.move);
    ...
    /**
     * The same as *applyMove*, but throws the error on illegible move
     * @param gameState
     * @param move
     */
    static move(gameState, move) {
        if (!this.applyMove(gameState, move))
    ...
    static applyMove(gameState, move) {
        const entry = this.validateMove(gameState, move);
        if (!entry)
            return false;
        ...
        if (gameState.enPassantTargetFile) {
            gameState.hash ^= enPassantNumbers[gameState.enPassantTargetFile];
        }
        // hash for side to move
        const nextSideToMove = this.nextSideToMove(gameState);
        ...
        switch (entry.type) {
            ...
            case 'move': {
                // xor out init pos
                gameState.hash ^= (0, HelperFunctions_1.getPieceNumber)(color, pieceType, move.start);
                // xor in new position
    ...
    static isKingChecked(gameState, side) {
        ...
        newGameState.moveHistory.push(this.buildHistoryEntry(newGameState, this.getMove({ ...kingPos }, { ...kingPos }), null, 'move'));
    ...
    */
    ...
    static buildHistoryEntry(gameState, move, destroyedPiece, actionType) {
