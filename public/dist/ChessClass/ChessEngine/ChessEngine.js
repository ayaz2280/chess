...
const GameStateHelperFunctions_3 = require("../GameStateHelperFunctions");
const console_1 = require("console");
const node_cache_1 = __importDefault(require("node-cache"));
const MoveUtils_1 = require("../Moves/MoveUtils");
class ChessEngine {
    ...
    static initGame(playerDetails, boardSetup, sideToMove) {
        ...
        (0, GameStateHelperFunctions_3.initGameStateHash)(gameState);
        this.flushAllCaches();
        return gameState;
    }
    ...
    static getPawnMoves(gameState, pos, types) {
        ...
        if (!types || types.includes('enPassant')) {
            ...
            if (enPassantPos) {
                ...
                entry = this.buildHistoryEntry(gameState, move, (0, GameStateHelperFunctions_1.getFigure)(gameState, (0, MoveUtils_1.getPositionRelativeTo)(enPassantPos, dir, { x: 0, y: -1 })), 'enPassant', { isPromotion: false });
                if (entry) {
                    moves.push(entry);
                }
            }
    ...
    static getCastlingMoves(gameState, kingPos) {
        ...
        if (!(0, GameStateHelperFunctions_1.isFirstMove)(gameState, kingPos) || this.isKingChecked(gameState, piece.getColor())) {
            return moves;
        }
        ...
        if (leftRookOnPlace) {
            ...
            if (castlingPossible) {
                const move = this.getMove(kingPos, (0, MoveUtils_1.getPositionRelativeTo)(kingPos, 'forward', { x: -2, y: 0 }));
    ...
    static getEnPassantPos(gameState, pos) {
        ...
        const moveOffset = (0, MoveUtils_1.getMoveOffset)(lastHistoryEntry.move, this.getDirection(gameState, lastHistoryEntry.piece));
    ...
    static canAttackSquare(gameState, attackerPos, squareToAttack, attackingOffset) {
        ...
        if (this.areAllies(piece, piece2))
            return false;
        const calculatedSquare = (0, MoveUtils_1.getPositionRelativeTo)(attackerPos, this.getDirection(gameState, piece), attackingOffset);
        if (!calculatedSquare || !(0, GameStateHelperFunctions_2.isSamePos)(calculatedSquare, squareToAttack))
            return false;
        return true;
    }
    ...
    static applyMoveDebug(gameState, move) {
        const entry = this.getLegalMoves(gameState, move.start).find(e => (0, GameStateHelperFunctions_2.isSameMove)(e.move, move));
        if (!entry)
            return false;
    ...
    static isKingChecked(gameState, side) {
        ...
        const newGameState = (0, GameStateHelperFunctions_3.cloneGameState)(gameState);
    ...
    */
    ...
    static simulateMove(gameState, move) {
        ...
        const entry = this.getMoves(newGameState, move.start).find(e => (0, GameStateHelperFunctions_2.isSameMove)(e.move, move));
        if (!entry)
            return null;
    ...
    static isSquareAttackedBy(gameState, square, attackerSide) {
        ...
        for (const enemyPos of attackerFigurePositions) {
            ...
            if (endPositions.find(endP => (0, GameStateHelperFunctions_2.isSamePos)(endP, square)))
                return true;
        }
        return false;
    }
    ...
    static buildHistoryEntry(gameState, move, destroyedPiece, actionType, promotionDetails) {
        ...
        if (actionType === 'castling') {
            ...
            const rookMove = this.getMove(rookPos, (0, MoveUtils_1.getPositionRelativeTo)(rookPos, 'forward', { x: isRightRook ? -2 : 3, y: 0 }));
        ...
        if (actionType === 'enPassant') {
            ...
            const capturedPawnSquare = (0, MoveUtils_1.getPositionRelativeTo)(move.end, this.getDirection(gameState, piece), moveOffset);
    ...
    static validateMove(gameState, move) {
        ...
        const entry = moves.find(mi => (0, GameStateHelperFunctions_2.isSameMove)(move, mi.move));
        if (!entry)
            return null;
        if (entry.type === 'castling') {
            const castlingEntry = entry;
    ...
    static placeFigure(gameState, pos, figure) {
        const board = gameState.board;
        ...
        try {
            ...
            (0, console_1.assert)(err instanceof Error);
            console.error(err);
            return false;
        }
    ...
    static removeFigure(gameState, pos) {
        const board = gameState.board;
